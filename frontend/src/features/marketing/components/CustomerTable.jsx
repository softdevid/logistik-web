...
function CustomerTable({ rows, onRowAction }) {
  return (
        ...
        <table className="w-full text-sm" style={{ minWidth: 980 }}>
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50/80">
              {CUSTOMER_CONFIG.columns.map((column) => (
                <th
                  key={column.key}
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td
                  colSpan={CUSTOMER_CONFIG.columns.length + 1}
                  className="px-4 py-10 text-center text-sm text-slate-400"
                >
                  Tidak ada data {CUSTOMER_CONFIG.label.toLowerCase()}
                </td>
              </tr>
            ) : (
              rows.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    <ActionMenu
                      items={CUSTOMER_CONFIG.rowActions}
                      onSelect={(action) => onRowAction(action, row)}
                    />
                  </td>
                </tr>
              ))
